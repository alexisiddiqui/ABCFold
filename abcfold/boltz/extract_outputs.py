...
class BoltzOutputExtractor:
    ...
    def extract_structures(self, seed: int, result_dir: Path, pred_dir: Path) -> None:
        ...
        output_seed_dir = (
            self.output_dir / self.protein_name / f"boltz_{self.protein_name}" / f"seed-{seed}"
        )
        ...
        try:
            # Find prediction data directory
            ...
            for i, cif_file in enumerate(cif_files):
                try:
                    ...
                    logging.warning(f"  Failed to load model {model_num}: {e}")
                    continue
    ...
    def extract_confidence_scores(self, seed: int, result_dir: Path, pred_dir: Path) -> None:
        ...
        try:
            ...
            if complex_plddt_scores:
                ...
                logging.info(
                    f"  Saved Boltz confidence scores ({len(complex_plddt_scores)} models): {confidence_file}"
                )
            else:
    ...
    def extract_ptm(self, seed: int, result_dir: Path, pred_dir: Path) -> None:
        ...
        try:
            ...
            if ptm_scores:
                ...
                chain_pair_pae_min_arr = self._convert_dict_list_to_2d_array(
                    chain_pair_pae_min_list
                )
    ...
    def extract_plddt(self, seed: int, result_dir: Path, pred_dir: Path) -> None:
        ...
        try:
            ...
            for plddt_file_path in plddt_files:
                try:
                    plddt_array, metadata = self._load_npz_or_json_with_metadata(
                        plddt_file_path, "plddt", metadata_keys=["atom_chain_ids"]
                    )
                    if plddt_array is not None:
                        ...
                        raise ValueError("Could not extract 'plddt' key from file")
                except Exception as e:
                    ...
                    try:
                        with open(pae_files[0], "r") as f:
                            ...
                            if atom_chain_ids is not None:
                                logging.debug("  Extracted atom_chain_ids from PAE file")
                    except Exception as e:
            ...
            if atom_chain_ids is None:
                conf_files = sorted(
                    pred_data_dir.glob(
                        f"confidence_{self.protein_name}_data_seed-{seed}_model_*.json"
                    ),
                    key=lambda x: int(x.name.split("model_")[-1].split(".json")[0]),
                )
                if conf_files:
                    try:
                        with open(conf_files[0], "r") as f:
                            ...
                            if atom_chain_ids is not None:
                                logging.debug("  Extracted atom_chain_ids from confidence file")
                    except Exception as e:
    ...
    def _load_npz_or_json_with_metadata(
        self, file_path: Path, data_key: str, metadata_keys: Optional[List[str]] = None
            ...
            for pae_file_path in pae_files:
                try:
                    ...
                    if pae_array is not None:
                        ...
                        raise ValueError("Could not extract 'pae' key from file")
                except Exception as e:
    ...
    def extract_pde(self, seed: int, result_dir: Path, pred_dir: Path) -> None:
        ...
        try:
            ...
            for pde_file_path in pde_files:
                try:
                    ...
                    if pde_array is not None:
                        ...
                        raise ValueError("Could not extract 'pde' key from file")
                except Exception as e:
            ...
            if all_pde:
                ...
                np.savez_compressed(pde_file, pde=pde_array, seed=seed, n_models=len(all_pde))
    ...
    def _check_for_errors(self) -> None:
        ...
        if error_files:
            ...
            log_output_dir = (
                self.output_dir / self.protein_name / f"boltz_{self.protein_name}" / "error_logs"
            )
...
Dependencies:
  ...
  Install: pip install numpy mdtraj
    ...
    parser.add_argument(
        "input_dir", type=str, help="Directory containing Boltz outputs (with boltz_* subdirectory)"
    )
    ...
    parser.add_argument(
        "--no-pde", dest="extract_pde", action="store_false", help="Skip PDE extraction"
    )
