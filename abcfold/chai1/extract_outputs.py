...
try:
    from Bio.PDB import MMCIFParser, PDBIO, Structure, Model
    HAS_BIOPYTHON = True
...
class ChaiOutputExtractor:
    ...
    def extract_structures(self, seed: int, seed_dir: Path) -> None:
        ...
        output_seed_dir = self.output_dir / self.protein_name / f"chai1_{self.protein_name}" / f"seed-{seed}"
        ...
        try:
            ...
            # Get all CIF files, sorted by model index
            cif_files = sorted(
            ...
            for cif_file in cif_files:
                try:
                    ...
                    model_num = cif_file.name.split("model_idx_")[-1].split(".cif")[0]
    ...
    def extract_confidence_scores(self, seed: int, seed_dir: Path) -> None:
        ...
        try:
            ...
            for scores_file in scores_files:
                try:
                    with np.load(scores_file, allow_pickle=True) as data:
                        aggregate_scores.append(float(data['aggregate_score'].item() if hasattr(data['aggregate_score'], 'item') else data['aggregate_score']))
                        has_clash_scores.append(float(data['has_inter_chain_clashes'].item() if hasattr(data['has_inter_chain_clashes'], 'item') else data['has_inter_chain_clashes']))
            ...
            if aggregate_scores:
                ...
                logging.info(f"  Saved Chai confidence scores ({len(aggregate_scores)} models): {confidence_file}")
            else:
        ...
        try:
            ...
            for scores_file in scores_files:
                try:
                    with np.load(scores_file, allow_pickle=True) as data:
                        aggregate_scores.append(float(data['aggregate_score'].item() if hasattr(data['aggregate_score'], 'item') else data['aggregate_score']))
                        ptm_scores.append(float(data['ptm'].item() if hasattr(data['ptm'], 'item') else data['ptm']))
                        iptm_scores.append(float(data['iptm'].item() if hasattr(data['iptm'], 'item') else data['iptm']))
            ...
            if ptm_scores:
                save_dict = {
                    'ranking_score': np.array(aggregate_scores, dtype=np.float32),  # Map aggregate_score to ranking_score
                ...
                per_chain_pair_iptm_arr = self._convert_per_chain_pair_list_to_array(per_chain_pair_iptm_list)
    ...
    def extract_contact_probs(self, seed: int, seed_dir: Path) -> None:
        ...
        try:
            ...
            if all_contact_probs:
                # Stack all models
                contact_probs_array = np.stack(all_contact_probs, axis=0)  # Shape: (n_models, n_tokens, n_tokens)
    ...
    def _convert_per_chain_pair_list_to_array(self, per_chain_pair_list: List) -> Optional[np.ndarray]:
    ...
    def _check_for_errors(self) -> None:
        ...
        if error_files:
            ...
            log_output_dir = self.output_dir / self.protein_name / f"chai1_{self.protein_name}" / "error_logs"
    ...
    def process(self, extract_structures: bool = True, extract_ptm: bool = True,
                ...
                extract_pae: bool = True, extract_contact_probs: bool = True) -> None:
...
Dependencies:
  ...
  Install: pip install numpy biopython
    ...
    parser.add_argument(
        'input_dir',
        type=str,
        help='Directory containing Chai outputs (with chai1_* subdirectory)'
    )
